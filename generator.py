...
async def generate_structured_response(req: GenerateRequest) -> dict[str, Any]:
    ...
    if isinstance(planned, dict) and "scene" in planned:
        ...
        raw_scene = planned.get("raw_json") if isinstance(planned.get("raw_json"), dict) else {}
    ...
    if ENABLE_INTERNAL_REVIEW:
        ...
        try:
            ...
            if review_raw_json:
                ...
                review_issues = [str(i) for i in issues if i]
                ...
                if not approved and revised_layout_plan:
                    ...
                    scene = build_scene_from_layout_plan(revised_layout_plan, req.prompt, current_scene=req.current_scene)
                    scene, scene_before_layout = _materialize_scene(scene, req.prompt)
                    review_applied = True
